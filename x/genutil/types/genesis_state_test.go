...
func TestValidateGenesisMultipleMessages(t *testing.T) {
	...
	txGen := l2app.MakeTestEncodingConfig().TxConfig
	...
	err = types.ValidateGenesis(genesisState, l2app.MakeTestEncodingConfig().TxConfig.TxJSONDecoder())
	require.Error(t, err)
}
...
func TestGenesisStateFromGenFile(t *testing.T) {
	...
	require.Equal(t, "link1wjfgxvmff06nqkzlnuhhdmm5d55lvffn5xym0g", bankGenesis.Balances[0].GetAddress().String())
